module.exports = Spudnik => {
	let lastPruned = new Date().getTime() - (Spudnik.Config.pruneInterval * 1000);
	...
	return {
		...
		prune: {
			...
			process: (msg, suffix) => {
				...
				if (timeSinceLastPrune > (Spudnik.Config.pruneInterval * 1000)) {
					if (!isNaN(parseInt(suffix, 10))) {
						let count = parseInt(suffix, 10);

						count++;

						if (count > Spudnik.Config.pruneMax) {
							count = Spudnik.Config.pruneMax;
						}
					...
					const wait = Math.floor(Spudnik.Config.pruneInterval - (timeSinceLastPrune / 1000));
					msg.channel.send({
						embed: {
							color: Spudnik.Config.defaultEmbedColor,
