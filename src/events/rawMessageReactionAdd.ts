...
export default class extends Event {
	...
	public async run(event: any): Promise<void> {
		if (event.guild_id) {
			...
			// Guild is on Blacklist, ignore.
			if (!SpudConfig.botListGuilds.includes(guild.id)) {
				...
				// Ignore if starboard isn't set up
				if (!(!starboardEnabled || !starboard || !starboardChannel)) {

					const channel: GuildChannel = guild.channels.get(event.channel_id);
					// Ignore NSFW channels
					if (!(channel as TextChannel).nsfw) {
						// Can't star items in starboard channel
						if (starboard !== channel) {
							...
							if (message.author.id !== event.user_id) {
								...
								if (!message.author.bot) {
									...
									// We don't support any emojis outside of the normal unicode ones
									if (currentEmojiKey.match(emojiRegex)) {
